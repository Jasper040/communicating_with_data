...
detail, meta = build_optimization_detail(scoped, brand, category, fit, size_group, int(target_qty), style)
if detail.empty:
    st.warning("No rows found for this profile.")
    st.stop()

# Hero callout — mirrors the business case (e.g. Malelions × Men's Tracksuits × …)
...
st.download_button(
    ...
    csv,
    file_name=f"optimization_detail_{brand}_{category}_{fit}_{size_group}.csv".replace(" ", "_"),
    mime="text/csv",
)
